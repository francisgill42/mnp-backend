...
class Maintenanceuser extends Model
{
    ...
    public function get_all_requests($from, $to, $status, $customer){
        $args = array();
        ...
        $requests = DB::table('maintenanceusers')
            ...
            ->select('maintenanceusers.*', 'statuses.id as status_id', 'statuses.status as status_text', 'users.id as user_id', 'users.company_name', 'users.role_id', 'users.customer_category_id', 'users.master', 'users.email', 'users.phone_number', 'users.mobile_number', 'users.ntn', 'users.address', 'users.state_id', 'users.city_id', 'users.IsActive')
            ->where($args)
    ...
    public function get_all_requests_by_customer($customer_id){
        $requests = DB::table('maintenanceusers')
            ->join('users', 'maintenanceusers.customer_id', '=', 'users.id')
            ->join('statuses', 'maintenanceusers.status', '=', 'statuses.id')
            ->select('maintenanceusers.*', 'statuses.status as status_text', 'users.id as user_id', 'users.company_name', 'users.role_id', 'users.customer_category_id', 'users.master', 'users.email', 'users.phone_number', 'users.mobile_number', 'users.ntn', 'users.address', 'users.state_id', 'users.city_id', 'users.IsActive')
